...
<Note>
  ...
  near the release. Last update: GH-8535 [Search
...
environment variables to make sessions work better). While not stricly correct,
this bandaid solution helps more than it hurts in most cases.
...
This sequence is already used by systemd and Zig and is now supported by
multiple terminals across Windows, Linux, and macOS. We hope given the
growing terminal support that more programs will start using this feature.
...
We've reworked our key bindings to be more consistent, based on the
[W3C key event code specification](https://www.w3.org/TR/uievents-code/).
This work should result in more predictable, working key bindings across
...
names as noted above. Previous `pyhsical:` keybinds continue to work but
...
for macOS users, but we stil plan to build alternate scripting choices in
the future.
